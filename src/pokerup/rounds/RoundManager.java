...
public class RoundManager implements GameConstants {
	...
	public int getTotalRoundBetAmount() {
		int totalBet = 0;
		for (TurnManager turnManager : listTurn) {
			System.out.println(turnManager.getPlayer().getPlayerName() +" << Bet Amount  : " + turnManager.getBetAmount());
			totalBet += turnManager.getBetAmount();
		}
		System.out.println(currentRound + " >>  Total Bet Amount  : " + totalBet);
		return totalBet;
	}
