...
import pokerup.utils.GameConstants.HAND_RANK;
...
public class WinnerManager {
	...
	public List<PlayerBean> generateWinnerPlayers() {
		...
		List<PlayerBean> listAllActivePlayers = playerManager
				.getAllAactivePlayersForWinning();
		// Sort winner players
		Collections.sort(listAllActivePlayers, new Comparator<PlayerBean>() {
			@Override
		...
		for (int i = 0; i < listAllActivePlayers.size(); i++) {
			...
			PlayerBean currentPlayer = listAllActivePlayers.get(i);
			if (!listWinnerPlayer.contains(currentPlayer)) {
				sameRankPlayer.add(currentPlayer);
				for (int j = i + 1; j < listAllActivePlayers.size(); j++) {
	...
	public void findWinnerPlayers() {
		...
		for (Winner player : listWinners) {
			System.out.println("\n Winner Player =  "
					+ player.getPlayer().getPlayerName() + " :: Amount : "
					+ player.getWinningAmount());
		}
	...
	public int getBestHandPlayer(PlayerBean playerBean1, PlayerBean playerBean2) {
		...
		if (playerBean1.getHandRank().equals(playerBean2.getHandRank())) {
			...
			if (playerBean1.getHandRank().equals(HAND_RANK.FULL_HOUSE)) {
				List<List<Card>> listOfListSameRankCardsForPlayer1 = generalHandManager
						.listAllSameRankCards(playerBean1.getMainHandCards());
				List<List<Card>> listOfListSameRankCardsForPlayer2 = generalHandManager
						.listAllSameRankCards(playerBean2.getMainHandCards());
				List<Card> listPlayer1Cards = new ArrayList<Card>();
				List<Card> listPlayer2Cards = new ArrayList<Card>();

				if (listOfListSameRankCardsForPlayer1.size() == 2
						&& listOfListSameRankCardsForPlayer2.size() == 2) {
					if (listOfListSameRankCardsForPlayer1.get(0).size() == 3) {
						listPlayer1Cards
								.addAll(listOfListSameRankCardsForPlayer1
										.get(0));
						listPlayer1Cards
								.addAll(listOfListSameRankCardsForPlayer1
										.get(1));
					...
					if (listOfListSameRankCardsForPlayer2.get(0).size() == 3) {
						listPlayer2Cards
								.addAll(listOfListSameRankCardsForPlayer2
										.get(0));
						listPlayer2Cards
								.addAll(listOfListSameRankCardsForPlayer2
										.get(1));
					...
					for (int i = 0; i < listPlayer1Cards.size(); i++) {
						if (listPlayer1Cards.get(i).getValue() == listPlayer2Cards
								.get(i).getValue()) {

						} else if (listPlayer1Cards.get(i).getValue() > listPlayer2Cards
								.get(i).getValue()) {
							return -1;
						} else if (listPlayer1Cards.get(i).getValue() < listPlayer2Cards
								.get(i).getValue()) {
